...
real(kind=8) :: p,Temp,T_heat,Lx,Ly,Lz,Lhalf_x,Lhalf_y,Lhalf_z,dt,dt_trans,dt_rot,t_max,& 
                E_trans_cent, E_rot_cent, E_pot_cent,adiab_cent,P_total(3),laser_range,laser_range_by2, &
	   ...
	   xj,xk,yj,yk,correl_j(4),correl_jk,x2_cent,y2_cent,z2_cent,xy_cent,disp2_cent(3),disp4_cent(3),percent,x,Omega_cent,J_cent,J_min,J_max,E_min,E_max,theta_i,phi_i,vtheta_i,vphi_i,sigma_J_cent,F_thresh_sq 
integer :: small_nx,small_ny,small_nz,N,Nt,i,j,k,N_places=0,MaxPairsPerAtom,N_neigh, &
	maximum_box,N_cent,file_number=0,num_j,num_jk,run_ind,num_of_runs
integer, allocatable :: Marker1(:),Marker2(:),List(:),position(:), &
                        number_density(:), num_in_cutoff(:)
...
real(kind=8), allocatable :: R(:,:),V(:,:),e(:,:),u(:,:),u_half(:,:),F(:,:),DisplaceList(:,:), &
		...
		E_trans(:),E_rot(:),x2(:),y2(:),z2(:),xy(:),J_trans(:,:),J_rot(:,:),displacement(:,:),Omega_scal_sq(:),J_requan(:), &
J_cent_vec(:),vphi_i_vec(:),F_scal_sq(:),diffus_coef(:,:)
...
!Bromine         257.2                    3.54
 ...
 const=8.643!17.97 != (3*epsilon)/(sigma^2) * (M*r_e^4)/(hbar^2) - M and r_e are for ^14 N_2 molecule; see the unit convention above
 sigma=3.31/1.1 !=sigma/r_e
 const2=(sigma**2)*const/12. != epsilon * (M*r_e^2)/(4*hbar^2)
 sigmasq=sigma**2
 cutoff=sigma*3.0! cutoff distance of the Lennard-Jones force 
 cutoffsq=cutoff**2
 pot_at_cutoff=const2*((sigmasq/cutoffsq)**6-(sigmasq/cutoffsq)**3)! value of the potential at cutoff distance

!F_thresh_sq=1.0D-6
!write(*,*) 'F_th_sq',F_thresh_sq
...
Nt=120000000! total number of timesteps
p=1. !pressure in atm
Temp=300. ! temperature in Kelvin
!!!!!!!!!DEFINING MOLECULAR MASS, BOND LENGTH AND MOMENT OF INERTIA

M_mol=28./28.!A mass of the 16^O_2 molecule (in units of mass of 14^N_2 molecule)
re_mol=1.1/1.1 !Distance between the 2 oxygen "force centers" (in units of the N_2 molecule bond length0)
...
allocate( diffus_coef(Nt/1000+1,3) )
!diffus_coef=0.


pol=(/0.,0.,1./) !Polarization direction of the pulse
P_kick=40. !Kick strength of the pulse
...
call initial_cond(p,Temp,small_nx,small_ny,small_nz, &
                   R_ini,e_ini,V_ini,u_ini,Lx,Ly,Lz,N,I_mol)
...
allocate(R(N,3),deltaR(N,3),DisplaceList(N,3),V(N,3),e(N,3),u(N,3),u_half(N,3), &
          ...
          lambda(N,3),Omega(N,3),zeros(N,3),Jbig(N,3),places1(N),places3(N,3), &
          position(N), &
	E_trans(N),E_rot(N),x2(N),y2(N),z2(N),xy(N),J_trans(N,3),J_rot(N,3),displacement(N,3),places_cent(N),Omega_scal_sq(N),J_requan(N), &
J_cent_vec(N),vphi_i_vec(N),F_scal_sq(N),num_in_cutoff(N) )
...
!open(600,file='ini_cond_6800_T300_all_centJ80_N2b.dat')
...
! open(60,file='final_cond_1e4_T300_0pct_centrifugeJ0_N2.dat')
! read(60,*)
! do j=1,N
!    read(60,*) R(j,:)
! end do
...
phi_i=0.
vtheta_i=0.
vphi_i=J_cent/(4*I_mol)

!call random_number(J_cent_vec)
!call random_number(vphi_i_vec)
!J_cent_vec=sigma_J_cent*sqrt(-2.*log(J_cent_vec))*cos(2.*pi*vphi_i_vec)+J_cent
!J_cent_vec=nint(J_cent_vec)
!vphi_i_vec=sqrt(J_cent_vec*(J_cent_vec+1))/(4*I_mol)


do i=1,N

   if (places_cent(i)) then
	e(i,1)=sin(theta_i)*cos(phi_i)
	e(i,2)=sin(theta_i)*sin(phi_i)
	e(i,3)=cos(theta_i)

	u(i,1)=vtheta_i*cos(theta_i)*cos(phi_i)-vphi_i*sin(phi_i)
	u(i,2)=vtheta_i*cos(theta_i)*sin(phi_i)+vphi_i*cos(phi_i)
	u(i,3)=-vtheta_i*sin(theta_i)
...
!Inserting a single molecule with V=v_rot:
!V(1,1)=vphi_i
...
!        call random_number(theta_J)
!        call random_number(phi_J)
!        call random_number(phi_r)
!        theta_J=2.*asin(sqrt(theta_J))
!        phi_J=2.*pi*phi_J
!        phi_r=2.*pi*phi_r

!        e_J=(/ sin(theta_J)*cos(phi_J),sin(theta_J)*sin(phi_J),cos(theta_J) /)
!        r_vec=(/ cos(theta_J)*cos(phi_J)*cos(phi_r)-sin(phi_J)*sin(phi_r), &
!             cos(theta_J)*sin(phi_J)*cos(phi_r)+cos(phi_J)*sin(phi_r), &
!             -sin(theta_J)*cos(phi_r) /)

!        J_cross_r=(/ e_J(2)*r_vec(3)-e_J(3)*r_vec(2), &
!                     e_J(3)*r_vec(1)-e_J(1)*r_vec(3), &
!                     e_J(1)*r_vec(2)-e_J(2)*r_vec(1) /)
!        v_vec=sigma_u*(J_cross_r/(sqrt(dot_product(J_cross_r,J_cross_r))))
...
! open(1011,file='cent_num_and_place_1e4_T100_10pct_centrifuge.dat')
! read(1011,*) N_cent
! do i=1,N
...
laser_range=(3./40./pi*Lx*Ly*Lz)**(1./3.)
zeros=0
places1=(R_ini(:,1)**2+R_ini(:,2)**2+R_ini(:,3)**2)<laser_range**2
places1=.true.
places3=spread(places1,2,3)
do j=1,N
   if (places1(j)) N_places=N_places+1
end do
!write(*,*) 'N_diff=',N_places
...
open(111,file='time_1e4_T300_all_centJ90_N2.dat') ! Time vector
write(111,*) 0.
do i=1000,Nt,1000
	write(111,*) i*dt
end do
...
!open(19,file='R_of_t_1e3_T300p10_all_centJ60_N2.xyz')
!write(19,*) 2*N
!write(19,*) 'Energy='
!do j=1,N
!   write(19,*) 'o    ',R(j,:)+0.5*re_mol*e(j,:)
...
!open(101,file='cent_num_and_place_1e4_T300_10pct_centrifugeJ60_N2echo12.dat')
!write(101,*) N_cent
!do i=1,N
...
open(2,file='energy_1e4_T300_all_centJ90_N2.dat')
write(2,*) sum(E_trans)/N,sum(E_rot)/N,sum(sqrt(E_rot/E_trans)) !Average translational and rotational energy

!open(12,file='disp2_1e4_T300_all_centJ80_N2.dat')
!open(13,file='disp2_std_1e4_T300_10pct_centrifugeJ60_N2echo12.dat')
!write(12,*) sum(displacement**2,1)/N

!open(14,file='diffus_coef_1e4_T300_all_centJ60_N2avg10.dat')
!write(14,*) sum(displacement*V,1)/N

!diffus_coef(1,:)=diffus_coef(1,:)+sum(displacement*V,1)/N

!open(222,file='V_dist_1e4_T300_all_centJ60_N2.dat') !Initial velocity distribution
!do j=1,N
!	write(222,*) V(j,:)
!end do
!close(222)
...
!z2=e(:,3)**2
!xy=e(:,1)*e(:,2)
!open(33,file='orient_1e4_T300_all_centJ60_N2avg10.dat')
...
!J_min=J_cent-1.
!J_max=J_cent+1.
!E_min=J_min*(J_min+1)/(32*I_mol)
!E_max=J_max*(J_max+1)/(32*I_mol)

!open(44,file='correl_1e4_T300_10pct_centrifugeJ60_N2echo12.dat')
!open(4414,file='num_correl_1e4_T300_10pct_centrifugeJ60_N2echo12.dat')
!Omega_cent=sqrt(J_cent*(J_cent+1))/(4*I_mol) 
! correl_j=0.
! correl_jk=0.
!num_j=0
!num_jk=0
...
!do j=1,N
!if (E_rot(j)>E_min .and. E_rot(j)<E_max) then
!     xj=e(j,1)
!     yj=e(j,2)
!     correl_j=correl_j+(/x2(j),y2(j),z2(j),xy(j) /)!(cos(atan2(yj,xj)))**2
!     num_j=num_j+1
!   do k=j+1,N
!	if (E_rot(k)>E_min .and. E_rot(k)<E_max) then
!	xk=e(k,1)
!
!	yk=e(k,2)
...
!end if
!end do
! correl_j=correl_j/(N-1)
! correl_jk=2*correl_jk/(N*(N-1))
!write(44,*) correl_j!, correl_jk
!write(4414,*) num_j!, num_jk
...
!open(3,file='ang_mom_trans_1e4_T300_all_centrifuge6_O2.dat')
!write(3,*) sum(J_trans,1)/N
!open(4,file='ang_mom_rot_1e4_T300_all_centJ60_N2avg10.dat')
!write(4,*) sum(J_rot,1)/N
!open(55,file='ang_mom_sq_rot_1e4_T300_all_centJ60_N2avg10.dat')
!write(55,*) sum(J_rot**2,1)/N

!open(444,file='ang_mom_rot_dist_1e4_T300_all_centJ90_N2.dat')
!do j=1,N
! 	write(444,*) J_rot(j,:)
!end do
!close(444)
...
!        adiab_cent=adiab_cent+sqrt(E_rot(j)/E_trans(j))
...
!end do
!write(2,*) E_trans_cent/N_cent,E_rot_cent/N_cent,adiab_cent/N_cent
...
! 	end do
 ...
 !		if (mod(i,500000)==0) then ! Velocity distribution is saved every million time steps
 ...
 !filename_V='V_dist_1e4_T300_all_centJ90_N2_'//file_number_char//'.dat'
 ...
 !filename_ang_mom='ang_mom_rot_dist_1e4_T300_all_centJ90_N2_'//file_number_char//'.dat'
...
Omega(:,2)=e(:,3)*u(:,1)-e(:,1)*u(:,3)
Omega(:,3)=e(:,1)*u(:,2)-e(:,2)*u(:,1)
Omega_scal_sq=Omega(:,1)**2+Omega(:,2)**2+Omega(:,3)**2
E_rot=0.5*I_mol*Omega_scal_sq
!E_pot=pot_energy_tot(R,e)/N
write(2,*) sum(E_trans)/N,sum(E_rot)/N,sum(sqrt(E_rot/E_trans))
...
!    xj=e(j,1)
!    yj=e(j,2)
!    correl_j=correl_j+(/x2(j),y2(j),z2(j),xy(j)/)!(cos(atan2(yj,xj)-Omega_cent*i*dt))**2
!    num_j=num_j+1
!   do k=j+1,N
!	if (E_rot(k)>E_min .and. E_rot(k)<E_max) then
!	xk=e(k,1)
...
!F_scal_sq=F(:,1)**2+F(:,2)**2+F(:,3)**2
!if (mod(i,1200)==0) then 

!Hartmann's requantization procedure: 
!J_requan=nint(sqrt(16*I_mol**2*Omega_scal_sq+0.25)-0.5)
!where (F_scal_sq<F_thresh_sq) 
!where (num_in_cutoff==0)

!u(:,1)=u(:,1)*sqrt(J_requan*(J_requan+1))/(4*I_mol*sqrt(Omega_scal_sq))
!u(:,2)=u(:,2)*sqrt(J_requan*(J_requan+1))/(4*I_mol*sqrt(Omega_scal_sq))
!u(:,3)=u(:,3)*sqrt(J_requan*(J_requan+1))/(4*I_mol*sqrt(Omega_scal_sq))

!end where
!Ilya's echo procedure:
!u=-u
...
!open(600,file='final_cond_1e4_T300_all_centJ80_N2.dat')
!write(600,4) p,Temp,N
!4 format('p= ',f6.2,'Temp= ',f6.2,'N= ',i5)
!do j=1,N
!   write(600,*) R(j,:)
...
subroutine initial_cond(p,Temp,small_nx,small_ny,small_nz,&
                         R_ini,e_ini,V_ini,u_ini,Lx,Ly,Lz,N,I_mol)
implicit none
real(kind=8), intent(in) :: p, Temp,I_mol !pressure in atmospheres, temperature in Kelvin
...
integer, intent(out) :: N !number of molecules, number of centrifuged molecules
real(kind=8) :: small_l, sigma_V, sigma_u,v_by_V,theta_J,phi_J,phi_r,e_J(3),r_vec(3),v_vec(3),J_cross_r(3) ! small_l - mean distance between molecules along any !dimension, sigma_V - translational temperature, sigma_u - rotational temperature
integer :: small_n,row, nx, ny, nz, i, x_int, clock, seed(1) 
...
num_in_cutoff(i)=num_in_cutoff(i)+1
	end if
...
delta_u=P*cos_beta_mat*(pol_mat-cos_beta_mat*e)/(2*I_mol)
...
sigma_V=3.62*sqrt(T_heat/300.*1./M_mol)
call random_number(n_mat)
